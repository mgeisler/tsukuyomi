...
fn scope_simple() -> Result<()> {
    let app: App = App::build(|mut scope| {
        scope.mount("/")?.done(|mut scope| {
            scope.at("/a")?.to(endpoint::reply("a"))?;
            scope.at("/b")?.to(endpoint::reply("b"))
        })?;

        scope.at("/foo")?.to(endpoint::reply("foo"))?;

        scope.mount("/c")?.at("/d")?.to(endpoint::reply("c-d")) //
    })?;
...
fn scope_nested() -> Result<()> {
    let app: App = App::build(|mut s| {
        // 0
        s.mount("/")?.done(|mut s| {
            s.at("/foo")?.to(endpoint::reply(""))?; // /foo
            s.at("/bar")?.to(endpoint::reply("")) // /bar
        })?;

        // 1
        s.mount("/baz")?.done(|mut s| {
            s.at("/")?.to(endpoint::reply(""))?; // /baz

            // 2
            s.mount("/")?
                .at("/foobar")? //
                .to(endpoint::reply("")) // /baz/foobar
        })?;

        s.at("/hoge")?.to(endpoint::reply("")) // /hoge
    })?;
...
fn failcase_duplicate_uri() -> Result<()> {
    let res: Result<App> = App::build(|mut s| {
        s.at("/path")?.get().to(endpoint::reply("a"))?; //
        s.at("/path")?.post().to(endpoint::reply("b"))
    });
    assert!(res.is_err());
    Ok(())
}
...
fn failcase_different_scope_at_the_same_uri() -> Result<()> {
    let res: Result<App> = App::build(|mut s| {
        s.at("/path")?.to(endpoint::reply("a"))?; //
        s.mount("/")?.at("/path")?.post().to(endpoint::reply("b")) //
    });
    assert!(res.is_err());
    Ok(())
}
...
fn experimental_api() -> Result<()> {
    let _app: App = App::build(|mut scope| {
        scope.at("/")?.to(endpoint::reply("hello"))?; //
        ...
        scope.fallback(endpoint::reply("fallback")) //
    })?;
